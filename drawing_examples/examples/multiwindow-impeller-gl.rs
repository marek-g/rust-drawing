...
fn main() -> Result<(), Box<dyn Error>> {
    let app = Application::new(
        ApplicationOptions::new()
            .with_title("Example: multiwindow-impeller-gl")
            .with_opengl_stencil_bits(8)
            .with_force_xwayland(true),
    )
    .unwrap();

    let _gl_window1_rc = setup_window::<ImpellerContextGl>("Window 1");
    let _gl_window2_rc = setup_window::<ImpellerContextGl>("Window 2");
